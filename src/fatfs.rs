...
use virtio_drivers::{Hal, device::blk::{SECTOR_SIZE, VirtIOBlk}, transport::Transport};

pub struct FatFsBlockDevice<H: Hal, T: Transport> {
    block_device: VirtIOBlk<H, T>,
    cursor: u64,
...
impl<H: Hal, T: Transport> Read for FatFsBlockDevice<H, T> {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        ...
        while buffer_cursor < buf.len() {
            ...
            self.block_device.read_blocks(current_block, &mut self.sector_buffer).unwrap();
...
impl<H: Hal, T: Transport> Write for FatFsBlockDevice<H, T> {
    fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
        ...
        while buffer_cursor < buf.len() {
            ...
            self.block_device.write_blocks(current_block, &self.sector_buffer).unwrap();
    ...
    fn seek(&mut self, pos: fatfs::SeekFrom) -> Result<u64, Self::Error> {
        match pos {
            ...
            fatfs::SeekFrom::End(offset) => {
                let total_size = self.block_device.capacity() * SECTOR_SIZE as u64;
